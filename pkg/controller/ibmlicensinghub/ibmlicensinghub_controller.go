...
// Add creates a new IBMLicensingHub Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New("ibmlicensinghub-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource IBMLicensingHub
	err = c.Watch(&source.Kind{Type: &operatorv1alpha1.IBMLicensingHub{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// Watch for changes to secondary resource Pods and requeue the owner IBMLicensingHub
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
		OwnerType:    &operatorv1alpha1.IBMLicensingHub{},
	})
	if err != nil {
		return err
	}
...
// blank assignment to verify that ReconcileIBMLicensingHub implements reconcile.Reconciler
var _ reconcile.Reconciler = &ReconcileIBMLicensingHub{}

// ReconcileIBMLicensingHub reconciles a IBMLicensingHub object
type ReconcileIBMLicensingHub struct {
