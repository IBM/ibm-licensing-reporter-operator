...
MARKDOWN_LINT_WHITELIST ?= https://quay.io/cnr,https://www-03preprod.ibm.com/support/knowledgecenter/SSHKN6/installer/3.3.0/install_operator.html,https://github.com/IBM/ibm-licensing-hub-operator/releases/download/
...
bundle: ## bundle zip file for certification
	...
	-mkdir bundle
	rm -rf bundle/*
	cp -r deploy/olm-catalog/ibm-licensing-hub-operator/${CSV_VERSION}/* bundle/
	cp deploy/olm-catalog/ibm-licensing-hub-operator/ibm-licensing-hub-operator.package.yaml bundle/
	cd bundle && zip ibm-licensing-hub-metadata ./*.yaml && cd ..
...
.PHONY: all build run install uninstall code-dev check lint test coverage multiarch-image csv clean help
